...
class MuZeroConfig:
    def __init__(self):
        ...
        self.max_moves = 9 * 9  # Maximum number of moves if game is not finished before
        ...
        self.num_unroll_steps = 9 * 9  # Number of game moves to keep for every batch element
        self.td_steps = 9 * 9  # Number of steps in the future to take into account for calculating the target value
...
class HollowNoGo:
    ...
    def reset(self):
        ...
        self.board[4][1] = Piece.HOLE.value
        self.board[4][2] = Piece.HOLE.value
        self.board[4][6] = Piece.HOLE.value
        self.board[4][7] = Piece.HOLE.value
        self.board[1][4] = Piece.HOLE.value
        self.board[2][4] = Piece.HOLE.value
        self.board[6][4] = Piece.HOLE.value
        self.board[7][4] = Piece.HOLE.value
        self.player = 1
    ...
    def get_observation(self):
        board_player1 = numpy.where(self.board == 1, 1.0, self.board)
        board_player2 = numpy.where(self.board == -1, 1.0, self.board)
    ...
    def check_legal_move(self, action, player):
        ...
        if abs(self.board[x][y]) == Piece.HOLE.value:
            return False
        
        if self.board[x][y] != Piece.EMPTY.value:
            return False
        
        self.board[x][y] = player

        if not self.check_liberty(x, y, player):
            self.board[x][y] = Piece.EMPTY.value
            return False
        if x > 0 and not self.check_liberty(x-1, y, -player):
            self.board[x][y] = Piece.EMPTY.value
            return False
        if x < self.board_size - 1 and not self.check_liberty(x+1, y, -player):
            self.board[x][y] = Piece.EMPTY.value
            return False
        if y > 0 and not self.check_liberty(x, y-1, -player):
            self.board[x][y] = Piece.EMPTY.value
            return False
        if y < self.board_size - 1 and not self.check_liberty(x, y+1, -player):
            self.board[x][y] = Piece.EMPTY.value
            return False
    ...
    def legal_actions(self):
        ...
        for action in range(self.board_size * self.board_size):
            if self.check_legal_move(action, self.player):
                l.append(action)
    ...
    def is_finished(self):
        for action in range(self.board_size * self.board_size):
            if self.check_legal_move(action, -1 * self.player):
                return False
        return True
    ...
    def render(self):
        for y in range(self.board_size):
            for x in range(self.board_size):
                if self.board[x][y] == Piece.EMPTY.value:
                    print("\u00B7", end=" ")
                elif self.board[x][y] == Piece.BLACK.value:
                    print("\u25CF", end=" ")
                elif self.board[x][y] == Piece.WHITE.value:
                    print("\u25CB", end=" ")
                elif self.board[x][y] == Piece.HOLE.value:
                    print(" ", end=" ")
                else:
                    print(" ", end=" ")
            print()
